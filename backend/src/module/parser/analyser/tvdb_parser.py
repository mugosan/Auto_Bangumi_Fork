...
def _last_aired_season(seasons: list[dict]) -> int:
    ...
    valid = [
        s for s in seasons if s.get("year") and int(s.get("year") or 0) <= current_year
    ]
    if not valid:
...
async def tvdb_parser(title: str, language: str, test: bool = False) -> TVDBInfo | None:
    ...
    if not tvdb_id:
        logger.warning(
            "[TVDB] Could not extract tvdb_id from search results for '%s'", title
        )
        _tvdb_cache[cache_key] = None
        return None
    ...
    ext_data = await _get(
        token, f"/series/{tvdb_id}/extended", {"meta": "translations", "short": "false"}
    )
