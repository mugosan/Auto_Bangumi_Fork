...
class TestGet:
    async def test_returns_json_on_success(self):
        ...
        with patch(
            "module.parser.analyser.tvdb_parser.get_shared_client",
...
class TestTvdbParser:
    ...
    async def test_auth_failure_returns_none(self):
        ...
        with (
            patch("module.conf.settings", settings),
            patch(
                "module.parser.analyser.tvdb_parser.get_shared_client",
                AsyncMock(return_value=client),
            ),
    ...
    async def test_successful_english_lookup(self):
        ...
        with (
            ...
            patch(
                "module.parser.analyser.tvdb_parser.save_image",
                return_value="/posters/test.jpg",
            ),
        ):
            result = await tvdb_parser("Test Anime", "en")
