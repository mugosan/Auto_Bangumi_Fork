...
MIGRATIONS = [
    ...
    (
        10,
        "backfill tvdb_id column on bangumi for installs predating TVDB support",
        [
    ...
    (
        11,
        "add id_source column to bangumi to record whether tvdb_id came from TVDB or TMDB",
        [
        ...
        for version, description, statements in MIGRATIONS:
            ...
            if "bangumi" in tables and version == 10:
                ...
                if "tvdb_id" in columns:
                    needs_run = False
            if "bangumi" in tables and version == 11:
                columns = [col["name"] for col in inspector.get_columns("bangumi")]
                if "id_source" in columns:
                    needs_run = False
            if needs_run:
                try:
                    with self.engine.connect() as conn:
