...
class TestTitleParser:
    ...
    async def test_tmdb_parser_prefers_real_tvdb_id(self):
        ...
        with patch.object(
            title_parser_module, "tmdb_parser", AsyncMock(return_value=info)
        ):
            title, season, year, poster_link, meta_id, id_source = (
    ...
    async def test_tmdb_parser_falls_back_to_tmdb_id_without_cross_reference(self):
        ...
        with patch.object(
            ...
            _, _, _, _, meta_id, id_source = await TitleParser.tmdb_parser(
                "Vinland Saga", 1, "zh"
            )
    ...
    async def test_tmdb_parser_no_match_returns_tmdb_source(self):
        with patch.object(
            title_parser_module, "tmdb_parser", AsyncMock(return_value=None)
        ):
            title, season, _, _, meta_id, id_source = await TitleParser.tmdb_parser(
                "Unknown Show", 1, "zh"
            )
